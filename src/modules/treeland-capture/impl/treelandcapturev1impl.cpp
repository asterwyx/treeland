#include "treelandcapturev1impl.h"
...
void handle_treeland_capture_context_v1_destroy(wl_client *client,
...
// void handle_treeland_capture_context_v1_copy(struct wl_client *client,
...
//         return;
//     }
//     struct wlr_buffer *wlrBuffer = static_cast<struct wlr_buffer *>(wl_resource_get_user_data(buffer));
//     auto qwBuffer = QWBuffer::from(wlrBuffer);
//     Q_EMIT context->copy(qwBuffer);
// }
...
{
    Q_ASSERT(wl_resource_instance_of(resource, &treeland_capture_context_v1_interface, &context_impl));
...
//         wl_client_post_implementation_error(client, "Another session or context in selection, could not create session.");
//         wl_client_post_no_memory(client);
//         delete session;
...
// void handle_treeland_capture_manager_v1_capture_once(struct wl_client *client,
//                                                      struct wl_resource *resource,
//                                                      uint32_t source_hint,
//                                                      struct wl_resource *mask,
//                                                      uint32_t with_cursor,
...
//     struct treeland_capture_context_v1 *capture_context =
//         new treeland_capture_context_v1;
...
//     if (!manager->enterSelectionMode(capture_context)) {
//         wl_client_post_implementation_error(client, "Another session or context in selection, could not capture once.");
//         wl_client_post_no_memory(client);
//         delete capture_context;
//         manager->leaveSelectionMode();
//         return;
...
//     capture_context->withCursor = with_cursor;
//     capture_context->sourceHint = source_hint;
//     Q_EMIT manager->captureOnce(capture_context);
// }

void treeland_capture_manager_bind(wl_client *client,
                                   void *data,
                                   uint32_t version,
                                   uint32_t id)
{
    treeland_capture_manager_v1 *manager =
        static_cast<treeland_capture_manager_v1 *>(data);
...
void handle_treeland_capture_session_v1_start(wl_client *client,
                                              wl_resource *resource)
{
    Q_UNUSED(client);
    struct treeland_capture_session_v1 *session = capture_session_from_resource(resource);
...
// void treeland_capture_context_v1::sendCaptureInfo(int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t sourceType)
// {
//     Q_ASSERT(resource);
//     treeland_capture_context_v1_send_capture_info(resource, x, y, width, height, sourceType);
// }

// void treeland_capture_context_v1::sendBuffer(uint32_t format, uint32_t width, uint32_t height, uint32_t stride)
// {
//     Q_ASSERT(resource);
//     treeland_capture_context_v1_send_buffer(resource, format, width, height, stride);
// }

// void treeland_capture_context_v1::sendBufferDone()
// {
//     Q_ASSERT(resource);
//     treeland_capture_context_v1_send_buffer_done(resource);
// }

// void treeland_capture_context_v1::sendFlags(uint32_t flags)
// {
//     Q_ASSERT(resource);
//     treeland_capture_context_v1_send_flags(resource, flags);
// }

// void treeland_capture_context_v1::sendReady()
// {
//     Q_ASSERT(resource);
//     treeland_capture_context_v1_send_ready(resource);
// }

// void treeland_capture_context_v1::sendFailed()
// {
//     Q_ASSERT(resource);
//     treeland_capture_context_v1_send_failed(resource);
// }

void handle_treeland_capture_manager_v1_get_context(wl_client *client, wl_resource *resource, uint32_t context)
{
    struct treeland_capture_manager_v1 *manager = capture_manager_from_resource(resource);
    Q_ASSERT(manager);
...
void handle_treeland_capture_context_v1_create_session(wl_client *client, wl_resource *resource, uint32_t session)
{
    struct treeland_capture_context_v1 *context = capture_context_from_resource(resource);
    Q_ASSERT(context);
...
void handle_treeland_capture_context_v1_select_source(wl_client *client, wl_resource *resource, uint32_t source_hint, uint32_t freeze, uint32_t with_cursor, wl_resource *mask)
{
    struct treeland_capture_context_v1 *context = capture_context_from_resource(resource);
    Q_ASSERT(context);
    if (source_hint) {
...
void handle_treeland_capture_context_v1_capture(wl_client *client, wl_resource *resource, uint32_t frame)
{
...
{
    Q_ASSERT(resource);
    treeland_capture_context_v1_send_source_ready(resource, region.x(), region.y(), region.width(),  region.height(), source_type);
}
...
void handle_treeland_capture_frame_v1_copy(wl_client *client, wl_resource *resource, wl_resource *buffer)
{
    treeland_capture_frame_v1 *frame = capture_frame_from_resource(resource);
    Q_ASSERT(frame);
    QWBuffer *qwBuffer = QWBuffer::from(buffer);
    if (!qwBuffer) {
